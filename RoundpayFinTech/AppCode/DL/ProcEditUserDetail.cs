        ...
        public object Call(object obj)
        {
            ...
            try
            {
                ...
                if (ds.Tables.Count > 1)
                {
                    ...
                    res.EmailID = ds.Tables[1].Rows[0]["_EmailID"].ToString();
                    res.MobileNo = ds.Tables[1].Rows[0]["_MobileNo"].ToString();
                    res.Name = ds.Tables[1].Rows[0]["_Name"].ToString();
                    res.OutletName = ds.Tables[1].Rows[0]["_OutletName"].ToString();
                    res.RoleID = Convert.ToInt32(ds.Tables[1].Rows[0]["_RoleID"]);
                    res.AADHAR = ds.Tables[1].Rows[0]["_AADHAR"].ToString();
                    res.PAN = ds.Tables[1].Rows[0]["_PAN"].ToString();
                    res.Address = ds.Tables[1].Rows[0]["_Address"].ToString();
                    res.GSTIN = ds.Tables[1].Rows[0]["_GSTIN"].ToString();
                    res.UserID = Convert.ToInt32(ds.Tables[1].Rows[0]["_ID"]);
                    res.ReferalID = Convert.ToInt32(ds.Tables[1].Rows[0]["_ReferalID"]);
                    res.Pincode = ds.Tables[1].Rows[0]["_PinCode"].ToString();
                    res.City = ds.Tables[1].Rows[0]["_City"].ToString();
                    res.IsGSTApplicable = Convert.ToBoolean(ds.Tables[1].Rows[0]["_IsGSTApplicable"]);
                    res.IsTDSApplicable = Convert.ToBoolean(ds.Tables[1].Rows[0]["_IsTDSApplicable"]);
                    res.DMRModelID = ds.Tables[1].Rows[0]["_DMRModelID"] is DBNull ? 0 : Convert.ToInt16(ds.Tables[1].Rows[0]["_DMRModelID"]);
                    res.StateID = Convert.ToInt32(ds.Tables[1].Rows[0]["_StateID"]);
