    ...
    public class TransactionHelper
    {
        ...
        public async Task<TransactionStatus> MatchResponse(int OID, RechargeAPIHit rechargeAPIHit, string AccountNoKey, bool IsValidation = false)
        {
            ...
            try
            {
                ...
                {
                    bool IsStatusFound = string.IsNullOrEmpty(rechargeAPIHit.aPIDetail.StatusName), IsVendorIDFound = string.IsNullOrEmpty(rechargeAPIHit.aPIDetail.VendorID), IsOperatorIDFound = string.IsNullOrEmpty(rechargeAPIHit.aPIDetail.LiveID), IsErrorCodeFound = false, IsMessageFound = false, IsRefKeyFound = string.IsNullOrEmpty(rechargeAPIHit.aPIDetail.RefferenceKey);
                    foreach (DataTable tbl in ds.Tables)
                    {
                        if (!IsStatusFound)
                        {
                            if (IsValidation)
                            {
                                if (tbl.Columns.Contains(rechargeAPIHit.aPIDetail.ValidationStatusKey))
                                {
                                    IsStatusFound = true;
                                    string StatusValue = tbl.Rows[0][rechargeAPIHit.aPIDetail.ValidationStatusKey].ToString().ToUpper();
                            ...
                            {
                                if (tbl.Columns.Contains(rechargeAPIHit.aPIDetail.StatusName))
                                {
                                    IsStatusFound = true;
                                    string StatusValue = tbl.Rows[0][rechargeAPIHit.aPIDetail.StatusName].ToString().ToUpper();
                        ...
                        {
                            if (tbl.Columns.Contains(rechargeAPIHit.aPIDetail.LiveID))
                            {
                                IsOperatorIDFound = true;
                                tstatus.OperatorID = tbl.Rows[0][rechargeAPIHit.aPIDetail.LiveID].ToString();
                            }
                        }
                        if (!IsVendorIDFound)
                        {
                            if (tbl.Columns.Contains(rechargeAPIHit.aPIDetail.VendorID))
                            {
                                IsVendorIDFound = true;
                                tstatus.VendorID = tbl.Rows[0][rechargeAPIHit.aPIDetail.VendorID].ToString();
                            }
                        }
                        if (!IsErrorCodeFound && tstatus.Status != RechargeRespType.SUCCESS)
                        {
                            if (tbl.Columns.Contains(rechargeAPIHit.aPIDetail.ErrorCodeKey))
                            {
                                IsErrorCodeFound = true;
                                tstatus.APIErrorCode = tbl.Rows[0][rechargeAPIHit.aPIDetail.ErrorCodeKey].ToString();
                            }
                        }
                        if (!IsMessageFound && tstatus.Status != RechargeRespType.SUCCESS)
                        {
                            if (tbl.Columns.Contains(rechargeAPIHit.aPIDetail.MsgKey))
                            {
                                IsMessageFound = true;
                                tstatus.APIMsg = tbl.Rows[0][rechargeAPIHit.aPIDetail.MsgKey].ToString();
                            }
                        }
                        if (!IsRefKeyFound && tbl.Columns.Contains(rechargeAPIHit.aPIDetail.RefferenceKey))
                        {
                            IsRefKeyFound = true;
                            tstatus.RefferenceID = tbl.Rows[0][rechargeAPIHit.aPIDetail.RefferenceKey].ToString();
                        }
                        if (IsStatusFound && IsVendorIDFound && IsOperatorIDFound && IsErrorCodeFound && IsMessageFound && IsRefKeyFound)
                            break;
                    }
                    if (tstatus.Status.In(RechargeRespType.PENDING, RechargeRespType.FAILED))
                    {
        ...
        public async Task<RechargeAPIHit> HitRechargeAPI(RechargeAPIHit rechargeAPI, TransactionServiceReq TransRequest, TransactionServiceResp TransResponse)
        {
            ...
            try
            {
                ...
                else if (rechargeAPI.aPIDetail.APICode == APICode.BILLAVENUE)
                {
                    ...
                    {
                        var HitReqResp = billAML.BillPaymentStatus(TransRequest.VendorID, TransRequest.TransactionReqID);
                        rechargeAPIHit.aPIDetail.URL = HitReqResp.Request;
                        rechargeAPIHit.Response = HitReqResp.Response;
                    }
                    else
            ...
            try
            {
                ...
                if (ds.Tables.Count > 0)
                {
                    ...
                    foreach (DataTable tbl in ds.Tables)
                    {
                        ...
                        if (!IsStatusFound && tbl.Columns.Contains(bBPSLog.aPIDetail.BillStatusKey))
                        {
                            IsStatusFound = true;
                            string StatusValue = tbl.Rows[0][bBPSLog.aPIDetail.BillStatusKey].ToString().ToUpper();
                        ...
                        if (!IsBillNoFound && tbl.Columns.Contains(bBPSLog.aPIDetail.BillNoKey))
                        {
                            IsBillNoFound = true;
                            bBPSResponse.BillNumber = tbl.Rows[0][bBPSLog.aPIDetail.BillNoKey].ToString().ToUpper();
                        }
                        if (!IsBillDateFound && tbl.Columns.Contains(bBPSLog.aPIDetail.BillDateKey))
                        {
                            IsBillDateFound = true;
                            bBPSResponse.BillDate = tbl.Rows[0][bBPSLog.aPIDetail.BillDateKey].ToString();
                        }
                        if (!IsDueDateFound && tbl.Columns.Contains(bBPSLog.aPIDetail.DueDateKey))
                        {
                            IsDueDateFound = true;
                            bBPSResponse.DueDate = tbl.Rows[0][bBPSLog.aPIDetail.DueDateKey].ToString();
                        }
                        if (!IsBillAmountKeyFound && tbl.Columns.Contains(bBPSLog.aPIDetail.BillAmountKey))
                        {
                            IsBillAmountKeyFound = true;
                            bBPSResponse.Amount = tbl.Rows[0][bBPSLog.aPIDetail.BillAmountKey].ToString().Trim();
                            IsStatusFound = true;
                            bBPSLog.Amount = Convert.ToDecimal(bBPSResponse.Amount ?? "0");
                        }
                        if (!IsCustomerNameFound && tbl.Columns.Contains(bBPSLog.aPIDetail.CustomerNameKey))
                        {
                            IsCustomerNameFound = true;
                            bBPSResponse.CustomerName = tbl.Rows[0][bBPSLog.aPIDetail.CustomerNameKey].ToString();
                        }
                        if (!IsMessageFound && tbl.Columns.Contains(bBPSLog.aPIDetail.MsgKey))
                        {
                            APIMsg = tbl.Rows[0][bBPSLog.aPIDetail.MsgKey].ToString();
                            IsMessageFound = true;
                        }
                        if (!IsRefKeyFound && tbl.Columns.Contains(bBPSLog.aPIDetail.RefferenceKey))
                        {
                            IsRefKeyFound = true;
                            bBPSResponse.RefferenceID = tbl.Rows[0][bBPSLog.aPIDetail.RefferenceKey].ToString();
                            bBPSLog.helper.RefferenceID = bBPSResponse.RefferenceID;
                        }
                        if (!IsErrorCodeFound && tbl.Columns.Contains(bBPSLog.aPIDetail.ErrorCodeKey))
                        {
                            IsErrorCodeFound = true;
                            APIErrorCode = tbl.Rows[0][bBPSLog.aPIDetail.ErrorCodeKey].ToString();
                        }
        ...
        private string GenerateRandomSessionNo(int length)
        {
