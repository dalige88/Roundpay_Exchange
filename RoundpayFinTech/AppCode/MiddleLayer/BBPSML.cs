    ...
    public class BBPSML
    {
        ...
        public async Task<BBPSResponse> FetchBillML(TransactionServiceReq _req)
        {
            ...
            if (_lr.LoginTypeID == LoginType.ApplicationUser)
            {
                ...
                if (tresp.ExactNess != EXACTNESS.Exact)
                {
                    bbpsResponse.IsEditable = true;
        ...
        public ValidateOuletModel CheckOutletStatusForServices(int LoginTypeID, int LoginID, int OID, int OutletID, string SPKey, string OTP, int RMode, int PartnerID, string Token)
        {
            ...
            if (LoginTypeID == LoginType.ApplicationUser && LoginID > 1)
            {
                ...
                if (RMode == RequestMode.SDK && (!CheckOutLetStatusResp.SCode.In(ServiceCode.AEPS, ServiceCode.MiniBank) || OutletID < 10000))
                {
                    resp.Statuscode = ErrorCodes.Minus1;
                    resp.Msg = "Invalid request to process";
                    return resp;
                ...
                if (CheckOutLetStatusResp.SCode.In(ServiceCode.AEPS, ServiceCode.MiniBank) && CheckOutLetStatusResp.APICode.In(APICode.FINGPAY, APICode.RPFINTECH, APICode.TPFINTECH) && CheckOutLetStatusResp.KYCStatus == KYCStatusType.COMPLETED && CheckOutLetStatusResp.AEPSStatus == UserStatus.ACTIVE)
                {
                    resp.Statuscode = ErrorCodes.One;
                    ...
                    if (resp.InterfaceType == AEPSInterfaceType.FINGPAY)
                    {
                        if (CheckOutLetStatusResp.SCode.In(ServiceCode.MiniBank, ServiceCode.AEPS))
                        {
                            if (RMode.In(RequestMode.APPS, RequestMode.SDK))
                            {
                            ...
                            else if (RMode == RequestMode.API)
                            {
                                ...
                                else
                                {
                                    ...
                                    resp.ResponseStatusForAPI.ErrorCode = resp.ErrorCode;
                                    resp.ResponseStatusForAPI.KYCStatus = CheckOutLetStatusResp.KYCStatus;
                                    resp.ResponseStatusForAPI.ServiceOutletID = OutletID.ToString();
                    ...
                    {
                        if (CheckOutLetStatusResp.SCode.In(ServiceCode.MiniBank, ServiceCode.AEPS) && RMode.In(RequestMode.APPS, RequestMode.SDK))
                        {
                    ...
                    if (RMode.In(RequestMode.APPS, RequestMode.SDK, RequestMode.API))
                    {
                        resp.ResponseStatusForAPI.Statuscode = resp.Statuscode;
                        resp.ResponseStatusForAPI.Msg = resp.Msg;
                        resp.ResponseStatusForAPI.ErrorCode = resp.ErrorCode;
                ...
                if (!CheckOutLetStatusResp.IsOutletRequired || (CheckOutLetStatusResp.IsOutletRequired && (CheckOutLetStatusResp.FixedOutletID ?? string.Empty).Length > 0 && (CheckOutLetStatusResp.SCode.In(ServiceCode.MoneyTransfer, ServiceCode.BBPSService))))
                {
                    ...
                    else
                    {
                        if (CheckOutLetStatusResp.SCode.Equals(ServiceCode.GenralInsurance))
                        {
                            ...
                            var GIMLResp = GIML.GenerateTokenAndURLForRedirection(CheckOutLetStatusResp, RMode);
                        ...
                        if (CheckOutLetStatusResp.SCode.Equals(ServiceCode.BBPSService) && !CheckOutLetStatusResp.APICode.In(APICode.RPFINTECH, APICode.TPFINTECH) && CheckOutLetStatusResp.OutletID > 0 && CheckOutLetStatusResp.KYCStatus == KYCStatusType.COMPLETED && RMode == RequestMode.API)
                        {
                            resp.ResponseStatusForAPI.Statuscode = ErrorCodes.One;
                        ...
                        if (CheckOutLetStatusResp.SCode.Equals(ServiceCode.MoneyTransfer) && RMode == RequestMode.API)
                        {
                            resp.ResponseStatusForAPI.Statuscode = ErrorCodes.One;
                            resp.ResponseStatusForAPI.Msg = nameof(ErrorCodes.Transaction_Successful).Replace("_", " ");
                            resp.ResponseStatusForAPI.ErrorCode = resp.ErrorCode;
                ...
                if (CheckOutLetStatusResp.IsOutletManual)
                {
                    ...
                    if (CheckOutLetStatusResp.SCode.In(ServiceCode.AEPS, ServiceCode.MiniBank))
                    {
                        ...
                        if (CheckOutLetStatusResp.APICode == APICode.FINGPAY)
                        {
                            resp.InInterface = true;
                            resp.InterfaceType = AEPSInterfaceType.FINGPAY;
                            if (CheckOutLetStatusResp.SCode.In(ServiceCode.MiniBank, ServiceCode.AEPS) && RMode == RequestMode.APPS)
                            {
                                resp.SDKType = AEPSInterfaceType.FINGPAY;
                                resp.InterfaceType = AEPSInterfaceType.NO;
                                resp.InInterface = false;
                ...
                var _OutletServicePlusReqModel = new OutletServicePlusReqModel
                {
                    ...
                    ServicPlusOTP = OTP ?? string.Empty,
                ...
                if (_OutletServicePlusReqModel.IsOnboarding)
                {
                    ...
                    if (respStatus.Statuscode == ErrorCodes.One)
                    {
                        ...
                        resp.ResponseStatusForAPI.ErrorCode = resp.ErrorCode;
                        resp.ResponseStatusForAPI.KYCStatus = CheckOutLetStatusResp.KYCStatus;
                        resp.ResponseStatusForAPI.ServiceOutletID = CheckOutLetStatusResp.SCode == ServiceCode.PSAService ? string.Empty : OutletID.ToString();
                        resp.ResponseStatusForAPI.ServiceStatus = UserStatus.APPLIED;
                        return resp;
                    }
                ...
                {
                    _OutletServicePlusReqModel.IsOnboardingStatusCheck = CheckOutLetStatusResp.APIOutletVerifyStatus == UserStatus.APPLIED || CheckOutLetStatusResp.APIOutletDocVerifyStatus == UserStatus.APPLIED || CheckOutLetStatusResp.AEPSStatus == UserStatus.APPLIED || CheckOutLetStatusResp.DMTStatus == UserStatus.APPLIED;
                ...
                if (!_OutletServicePlusReqModel.IsOnboardingStatusCheck)
                {
                    ...
                    else if (CheckOutLetStatusResp.SCode.In(ServiceCode.AEPS, ServiceCode.MiniBank))
                    {
                        ...
                        var IsPanel = RMode == RequestMode.PANEL;
                        if (_OutletServicePlusReqModel.IsAEPSServicePlus || IsPanel || RMode == RequestMode.API)
                        {
                            IOnboardingML onboardingML = new OnboardingML(_accessor, _env);
                            ...
                            if (sPlusResp.Statuscode == ErrorCodes.One)
                            {
                                ...
                                else
                                {
                                    ...
                                    resp.ErrorCode = ErrorCodes.Transaction_Successful;
                                    resp.IsConfirmation = false;
                                    resp.CommonStr2 = sPlusResp.AEPSURL;
                                    resp.Statuscode = ErrorCodes.One;
                        ...
                        else if (!_OutletServicePlusReqModel.IsAEPSServicePlus)
                        {
                            ...
                            if (_apiRes.Statuscode == ErrorCodes.One)
                            {
                                ...
                                if (CheckOutLetStatusResp.APICode.In(APICode.MAHAGRAM, APICode.ROUNDPAY) && RMode.In(RequestMode.APPS, RequestMode.SDK))
                                {
