...
namespace RoundpayFinTech.AppCode.MiddleLayer
{
    public class PaymentGatewayML : IPaymentGatewayML, IUPIPaymentML
    {
        ...
        public PGModelForRedirection IntiatePGTransactionForWeb(int UserID, int Amount, int UPGID, int OID, int WalletID, string Domain, string VPA)
        {
        ...
        public async Task SavePGTransactionLog(int PGID, int TID, string Log, string TransactionID, string Checksum, int RequestMode, bool IsRequestGenerated, int Amount, string VendorID)
        {
            try
            {
        ...
        public CollectUPPayResponse InitiateUPIPaymentForWeb(int UserID, int Amount, int UPGID, int OID, int WalletID, string UPIID)
        {
            ...
            if (procRes.Statuscode == ErrorCodes.One)
            {
                ...
                if (res.Statuscode == ErrorCodes.One)
                {
                    ...
                    var resUpdate = (ResponseStatus)procU.Call(new UpdatePGTransactionRequest
                    {
                        Browser = _rinfo.GetBrowserFullInfo(),
                        LiveID = res.BankRRN,
        ...
        public ResponseStatus UpdateUPIPaymentStatus(UpdatePGTransactionRequest request)
        {
            ...
            IProcedure procU = new ProcUpdatePaytmTransaction(_dal);
            return (ResponseStatus)procU.Call(request);
        }
