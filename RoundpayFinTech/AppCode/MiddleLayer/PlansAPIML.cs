...
namespace RoundpayFinTech.AppCode.MiddleLayer
{
    public class PlansAPIML : IMplan, IRoundpayPlan, IPlanAPIPlan, ICyrusAPIPlan, IVastWebPlan, IMyPlanAPI
    {
        ...
        public PlansAPIML(IHttpContextAccessor accessor, IHostingEnvironment env, IEFTCommonH IEFTCommonH)
        {
            _accessor = accessor;
            _env = env;
            _c = new ConnectionConfiguration(_accessor, _env);
        ...
        public List<RNPRechPlansPanel> GetRNPRechargePlans(int oid, int circleId)
        {
            ...
            bool IsLivePlan = _IEFTCommonH.GetApplicationSettings()._IsRechargePlansAPIOnly == null ? false : Convert.ToBoolean(_IEFTCommonH.GetApplicationSettings()._IsRechargePlansAPIOnly);
            if (IsLivePlan)
            {
        ...
        private List<RNPRechPlansPanel> GetLiveRechargePlans(int oid, int circleId)
        {
            ...
            if (data.Statuscode == ErrorCodes.One)
            {
                ...
                if (data.APICode.Equals(PlanType.Roundpay))
                {
                    ...
                    {
                        CommonStr = "BulkInsertion | " + data.APICode + " | " + circleId,
        ...
        {
            return _IEFTCommonH.RNPRechargePlans_EF(o, c);
        }
        ...
        private RNPDTHCustInfo HitDthCustInfoAPI(string APICode, string URL, string Acc)
        {
            ...
            if (APICode.Equals(PlanType.MPLAN))
            {
                if (!string.IsNullOrEmpty(webResult))
                {
                    ...
                    else
                    {
                        ...
                        resp.AccountNo = dthCustInfoRes.Tel;
                        resp.CustomerName = dthCustInfoRes.Records[0].CustomerName;
                        resp.Balance = dthCustInfoRes.Records[0].Balance;
                        resp.PlanName = dthCustInfoRes.Records[0].PlanName;
                        resp.StatusCode = ErrorCodes.One;
                        resp.Msg = "Success";
                    }
                }
            ...
            else if (APICode.Equals(PlanType.PLANAPI))
            {
                if (!string.IsNullOrEmpty(webResult))
                {
                    ...
                    {
                        resp.AccountNo = Acc;
                        resp.CustomerName = dthCustInfoRes.DATA.Name;
                        resp.Balance = dthCustInfoRes.DATA.Balance;
                        resp.PlanName = dthCustInfoRes.DATA.Plan;
                        resp.StatusCode = ErrorCodes.One;
                        resp.Msg = "Success";
                    }
                }
            ...
            else if (APICode.Equals(PlanType.VASTWEB))
            {
                if (!string.IsNullOrEmpty(webResult))
                {
                    ...
                    resp.AccountNo = Acc;
                    resp.CustomerName = dthCustInfoRes.Response.SubscriberName;
                    resp.Balance = dthCustInfoRes.Response.SubsBalance;
                    resp.PlanName = dthCustInfoRes.Response.SchemeName;
                    resp.StatusCode = ErrorCodes.One;
                    resp.Msg = "Success";
                }
                else
        ...
        public string GetRefferalContent(int id)
        {
            return _IEFTCommonH.GetRefferalContent(id);
        }
