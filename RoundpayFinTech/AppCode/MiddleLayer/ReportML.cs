...
namespace RoundpayFinTech.AppCode.MiddleLayer
{
    public partial class ReportML : IReportML, IRechargeReportML, IDMRReportML, IRefundReportML, IAppReportML, IOutletML, IBillFetchReportML
    {
        ...
        public WalletRequest GetWalletRequestDetail(WalletRequest req)
        {
            req.LoginID = _lr.UserID;
            req.LoginTypeID = _lr.LoginTypeID;
            IProcedure _IProc = new ProcGetSettlementReport(_dal);
            return (WalletRequest)_IProc.Call(req);
        }
    ...
    public partial class ReportML : IInvoiceReportML
    {
        ...
        public void LoopDMTTransactionStatus()
        {
            int LastID = 0;
            int newLastID;
        StartHere:
            ...
            if (res.Count > 0)
            {
                foreach (var item in res)
                {
                    ...
                    var _ = rml.CheckStatusDMRAsync(item.CommonInt2, item.CommonStr ?? string.Empty);
                }
                if (LastID != newLastID)
                {
                    goto StartHere;
