    ...
    public class SDKML
    {
        ...
        public SDKDetailResponse GetSDKDetail(SDKRequest sDKRequest)
        {
            ...
            if (sdkResponse.Statuscode == ErrorCodes.One)
            {
                ...
                var onboadresp = bbpsML.CheckOutletStatusForServices(LoginType.ApplicationUser, sDKRequest.UserID ?? 0, procres.OID, sDKRequest.OutletID ?? 0, string.Empty, string.Empty, RequestMode.SDK, sDKRequest.PartnerID ?? 0, sDKRequest.Token ?? string.Empty);
        ...
        public async Task<SDKDetailResponse> InitiateMiniBank(SDKIntitateRequest sDKRequest)
        {
            ...
            {
                BBPSML bbpsML = new BBPSML(_accessor, _env, false);
                var onboadresp = bbpsML.CheckOutletStatusForServices(LoginType.ApplicationUser, sDKRequest.UserID ?? 0, procres.OID, sDKRequest.OutletID ?? 0, string.Empty, string.Empty, RequestMode.SDK, sDKRequest.PartnerID ?? 0, sDKRequest.Token);
        ...
        public async Task<SDKDetailResponse> AEPSWithdrawal(SDKIntitateRequest sDKRequest)
        {
            ...
            if (sdkResponse.Statuscode == ErrorCodes.One)
            {
                ...
                {
                    withRes = await deviceMl.Withdrawl(sDKRequest.pidData, sDKRequest.Aadhar, sDKRequest.BankIIN, sDKRequest.SDKType ?? 0, sDKRequest.Amount ?? 0, sDKRequest.UserID ?? 0, sDKRequest.OutletID ?? 0, RequestMode.SDK, procres.OID, sDKRequest.IMEI, sDKRequest.Lattitude, sDKRequest.Longitude).ConfigureAwait(false);
                }
                else
                {
                    withRes = await deviceMl.Aadharpay(sDKRequest.pidData, sDKRequest.Aadhar, sDKRequest.BankIIN, sDKRequest.SDKType ?? 0, sDKRequest.Amount ?? 0, sDKRequest.UserID ?? 0, sDKRequest.OutletID ?? 0, RequestMode.SDK, procres.OID, sDKRequest.IMEI, sDKRequest.Lattitude, sDKRequest.Longitude).ConfigureAwait(false);
                }
        ...
        public async Task<SDKDetailResponse> GetBalanceAEPS(SDKIntitateRequest sDKRequest)
        {
            ...
            if (sdkResponse.Statuscode == ErrorCodes.One)
            {
                ...
                IDeviceML deviceMl = new DeviceML(_accessor, _env, false);
                var balRes = await deviceMl.CheckBalance(sDKRequest.pidData, sDKRequest.Aadhar, sDKRequest.BankIIN, sDKRequest.SDKType ?? 0, sDKRequest.UserID ?? 0, sDKRequest.OutletID ?? 0, RequestMode.SDK, procres.OID, sDKRequest.IMEI, sDKRequest.Lattitude, sDKRequest.Longitude).ConfigureAwait(false);
        ...
        public SDKDetailResponse GetMiniStatement(SDKIntitateRequest sDKRequest)
        {
            ...
            if (sdkResponse.Statuscode == ErrorCodes.One)
            {
                ...
                IDeviceML deviceMl = new DeviceML(_accessor, _env, false);
                var miniStatementResponse = deviceMl.MiniStatement(sDKRequest.pidData, sDKRequest.Aadhar, sDKRequest.BankName, sDKRequest.BankIIN, sDKRequest.SDKType ?? 0, sDKRequest.UserID ?? 0, sDKRequest.OutletID ?? 0, RequestMode.SDK, procres.OID, string.Empty, sDKRequest.IMEI, sDKRequest.Lattitude, sDKRequest.Longitude);
