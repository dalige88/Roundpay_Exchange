    ...
    public sealed class TransactionML : ITransactionML
    {
        ...
        public async Task<TransactionResponse> DoTransaction(_RechargeAPIRequest req)
        {
            ...
            if (validateRes.IsBBPS)
            {
                ...
                if (req.FetchBillID == 0 && validateRes.IsBilling && string.IsNullOrEmpty(req.RefID))
                {
            ...
            try
            {
                if (validateRes.CircleValidationType == CValidationType && ApplicationSetting.LookUpType != LookupAPIType.NoLOOKUP)
                {
