        ...
        public async Task<BBPSResponse> AxisBankBillFetchRequest(BBPSLog bbpsLog)
        {
            ...
            var reqObj = new AxisBankBillFetchReqModel
            {
                ...
                agent = new AxisBankAgent
                {
                    ...
                    ifsc = string.Empty,
                    imei = (bbpsLog.APIReqHelper.InitChanel ?? string.Empty).Equals("MOB") ? "521186783342337" : string.Empty,
                                    ...
                                    else
                                    {
                                        if (bRespFetch.data.fetchAPIStatus.In("Acknowledged", "Retrying", "Active"))
                                        {
                                            ...
                                            if (i >= LoopTill)
                                            {
                                                ...
                                                billResponse.ErrorMsg = billResponse.Msg = bbpsLog.helper.Reason = billResponse.ErrorMsg.Replace("{REPLACE}", bRespFetch.statusMessage);
        ...
        public HitRequestResponseModel AxisBankBillValidation(BBPSPaymentRequest bBPSRequest, out string context)
        {
            ...
            var reqObj = new AxisBankBillFetchReqModel
            {
                ...
                agent = new AxisBankAgent
                {
                    ...
                    ifsc = string.Empty,
                    imei = (bBPSRequest.InitChanel ?? string.Empty).Equals("MOB") ? guid.ToString().Replace("-", "").Substring(0, 16) : string.Empty,
