...
namespace RoundpayFinTech.AppCode.ThirdParty.HotelAPI
{
    public class HotelApiML : ITrkTravalHotelMLAPI
    {
        ...
        private async Task<HotelModelResponce> HotelApiTokenAuth(string EndUserIP)
        {
            ...
            TrkTvlSession = "16a029e7-8897-4bce-a4e8-1e42dff19201";
            TrkTvlExpiry = Convert.ToDateTime("17-11-2021 11:32:11");
            model.TokenId = "16a029e7-8897-4bce-a4e8-1e42dff19201";
        ...
        public async Task<TekTvlErrorModel> SyncHotelDestinationApiDAta(string ip)
        {
            ...
            if (!string.IsNullOrEmpty(TrkTvlSession) && TrkTvlExpiry > DateTime.Now)
            {
                ...
                if (CityResp.Status == "1")
                {
                    ...
                    Auth.Error = (TekTvlErrorModel)_proc.Call(CityResp);
                }
            }
            return Auth.Error;
        }
        public async Task<TekTvlErrorModel> SyncHotelMasterApiDAta(string ip, string cityId)
        {
            ...
            if (!string.IsNullOrEmpty(TrkTvlSession) && TrkTvlExpiry > DateTime.Now)
            {
                var _hotelMasterSetting = new TekTvlHotelMasterApiRequest()
                {
                    TokenId = TrkTvlSession,
                    EndUserIp = ip,
                    CityId = cityId,
        ...
        private TvkHotelResponseRoot DeserializeTvkHotelXmlObject(string xmlStr)
        {
            ...
            var res = JsonConvert.DeserializeObject<TvkHotelResponseRoot>(jsonStr);
            return res;
        }
