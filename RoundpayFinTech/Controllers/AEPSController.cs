...
namespace RoundpayFinTech.Controllers
{
    public class AEPSController : Controller
    {
        ...
        public async Task<IActionResult> MosambeeStatusCheck()
        {
            ...
            var mbResp = await miniBankML.MakeMiniBankTransaction(new MiniBankTransactionServiceReq
            {
                AmountR = Validate.O.IsNumeric((mosambeeResp.amount ?? string.Empty).Replace(".", "")) ? Convert.ToDecimal(mosambeeResp.amount) : 0M,
                APIOpCode = mosambeeResp.transactionType.ToString(),
                TXNType = mosambeeResp.transactionType.ToString(),
                APICode = APICode.MOSAMBEE,
                LoginID = 1,
                OutletIDSelf = 0,
                RequestModeID = RequestMode.API,
                VendorID = mosambeeResp.transactionId,
                BankIIN = mosambeeResp.transactionId,
                BCID = mosambeeResp.merchantId,
                TerminalID = mosambeeResp.terminalId,
                OutletID = mosambeeResp.businessName,
                RequestURL = "AEPS/Mosambee/Updatestatus?" + resp.ToString()
            }).ConfigureAwait(false);
            if (mbResp.Statuscode == ErrorCodes.One)
            {
                var sttaus = mosambeeResp.responseCode == RechargeRespType._SUCCESS ? RechargeRespType.SUCCESS : RechargeRespType.PENDING;
                ...
                {
                    TID = Validate.O.IsNumeric((mbResp.TransactionID ?? string.Empty)) ? Convert.ToInt32(mbResp.TransactionID) : 0,
                    VendorID = mbResp.VendorID,
                    LT = 1,
                    LoginID = 1,
                    LiveID = mosambeeResp.transactionId,
                    Statuscode = sttaus,
                    RequestPage = "Callback",
