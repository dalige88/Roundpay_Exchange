...
namespace RoundpayFinTech.Controllers
{
    ...
    public partial class AppController : Controller
    {
        ...
        private readonly IEFTCommonH _IEFTCommonH;

        public AppController(IHttpContextAccessor accessor, IHostingEnvironment env, IEFTCommonH IEFTCommonH)
        {
            _accessor = accessor;
            _env = env;
            appML = new AppML(_accessor, _env);
            _IEFTCommonH = IEFTCommonH;
        }
        ...
        public async Task<IActionResult> CallOnboarding([FromBody] AppOnboardCheckReq onboardCheckReq)
        {
            ...
            if (appResp.Statuscode == ErrorCodes.One)
            {
                if (!appResp.IsPasswordExpired)
                {
                    ...
                    var onboadresp = bbpsML.CheckOutletStatusForServices(onboardCheckReq.LoginTypeID, onboardCheckReq.UserID, onboardCheckReq.OID, appResp.GetID, string.Empty, onboardCheckReq.OTP, RequestMode.APPS, 0, string.Empty);
        ...
        public async Task<IActionResult> GetRealLapuCommission([FromBody] AppMyCommReq appMyCommReq)
        {
            ...
            if (appResp.Statuscode == ErrorCodes.One)
            {
                if (!appResp.IsPasswordExpired)
                {
                    ...
                    appResponse.Msg = ErrorCodes.SUCCESS;
                    ISlabML slabML = new SlabML(_accessor, _env, false);
                    appResponse.slabDetail = await slabML.GetLapuRealComm(_filter);
                }
            }
            else
            {
        ...
        public async Task<IActionResult> GetCalculatedCommission([FromBody] AppMyCommReq appMyCommReq)
        {
            ...
            if (appResp.Statuscode == ErrorCodes.One)
            {
                if (!appResp.IsPasswordExpired)
                {
                    ...
                    appResponse.Msg = ErrorCodes.SUCCESS;
                    ISlabML slabML = new SlabML(_accessor, _env, false);
                    appResponse.commissionDisplay = await slabML.GetDisplayCommission(_filter);
                }
            }
            else
            {
        ...
        public async Task<IActionResult> RechSimplePlan([FromBody] AppSimplePlanReq appSimplePlanReq)
        {
            ...
            {
                await Task.Delay(0);
                var plans = new PlansAPIML(_accessor, _env, _IEFTCommonH);
        ...
        public async Task<IActionResult> MoveToWallet([FromBody] AppMoveToWalletReq appRequest)
        {
            ...
            if (appResponse.Statuscode == ErrorCodes.One)
            {
                var commonReq = new CommonReq
                {
                    ...
                    CommonStr2 = appRequest.TransMode
                };
        ...
        public async Task<IActionResult> GetBalanceAEPS([FromBody] AEPSBalanceRequest balanceReq)
        {
            ...
            if (appResp.Statuscode == ErrorCodes.One)
            {
                ...
                if (!appResp.IsPasswordExpired)
                {
                    ...
                    var balRes = await deviceMl.CheckBalance(balanceReq.pidData, balanceReq.Aadhar, balanceReq.BankIIN, balanceReq.InterfaceType, balanceReq.UserID, appResp.GetID, RequestMode.APPS, 0, balanceReq.IMEI, balanceReq.Lattitude, balanceReq.Longitude).ConfigureAwait(false);
        ...
        public async Task<IActionResult> AEPSWithdrawal([FromBody] AEPSBalanceRequest balanceReq)
        {
            ...
            if (appResp.Statuscode == ErrorCodes.One)
            {
                ...
                {
                    IDeviceML deviceMl = new DeviceML(_accessor, _env, false);
                    var withRes = await deviceMl.Withdrawl(balanceReq.pidData, balanceReq.Aadhar, balanceReq.BankIIN, balanceReq.InterfaceType, balanceReq.Amount, balanceReq.UserID, appResp.GetID, RequestMode.APPS, 0, balanceReq.IMEI, balanceReq.Lattitude, balanceReq.Longitude).ConfigureAwait(false);
        ...
        public async Task<IActionResult> Aadharpay([FromBody] AEPSBalanceRequest balanceReq)
        {
            ...
            if (appResp.Statuscode == ErrorCodes.One)
            {
                ...
                {
                    IDeviceML deviceMl = new DeviceML(_accessor, _env, false);
                    var withRes = await deviceMl.Aadharpay(balanceReq.pidData, balanceReq.Aadhar, balanceReq.BankIIN, balanceReq.InterfaceType, balanceReq.Amount, balanceReq.UserID, appResp.GetID, RequestMode.APPS, 0, balanceReq.IMEI, balanceReq.Lattitude, balanceReq.Longitude).ConfigureAwait(false);
                    aEPSWithResponse.Statuscode = withRes.Statuscode;
                    aEPSWithResponse.Msg = withRes.Msg;
                    if (withRes.Statuscode == ErrorCodes.One)
                    {
        ...
        public async Task<IActionResult> BankMiniStatement([FromBody] AEPSBalanceRequest balanceReq)
        {
            ...
            if (appResp.Statuscode == ErrorCodes.One)
            {
                ...
                {
                    IDeviceML deviceMl = new DeviceML(_accessor, _env, false);
                    var miniSTMTRes = deviceMl.MiniStatement(balanceReq.pidData, balanceReq.Aadhar, balanceReq.BankName, balanceReq.BankIIN, balanceReq.InterfaceType, balanceReq.UserID, appResp.GetID, RequestMode.APPS, 0, SPKeys.AepsMiniStatement, balanceReq.IMEI, balanceReq.Lattitude, balanceReq.Longitude);
        ...
        public IActionResult CheckFlagsEmail([FromBody] AppSessionReq appSessionReq)
        {
            ...
            if (appResp.Statuscode == ErrorCodes.One)
            {
                ...
                appResponse.IsEmailVerified = ApplicationSetting.IsEmailVefiricationRequired && ml.IsEMailVerified(appSessionReq.UserID) ? true : false;
                appResponse.Statuscode = ErrorCodes.One;
                appResponse.Msg = ErrorCodes.SUCCESS;
            }
            else
