class product {
    ...
    constructor() {
        ...
        $('#ddlCategory').change(e => { bindCategoryLevel1(e, 'ddlLevel1'); this.bindBrand(e, 'ddlBrand'); this.bindFilters(); });
            ...
            for (var i = 0; i < totalFile; i++) {
                ...
                if (divId === '') {
                    $(`#${selectElement}`).parent('div').append(`<div class="previewImage float-left text-center">
                                                                    ...
                                                                    <img src="${_src}" data-index-Id="${count === 0 ? i : count}">
                                                                 </div>`);
                }
                else {
    ...
    bindFilters() {
        $.post('/List-Filter-Option', { sid: 0 })/*$('#ddlLevel2').val() })*/
            .done(result => {
            ...
            .done(result => {
                ...
                for (let i = 0; i < result.length; i++) {
                    ...
                    $('#imagePreview').append(`<div class="previewImage float-left text-center">
                                             ...
                                             <span class="close fa fa-times"></span>
                                             <img src="/Image/Products/${result[i].replace("-1x", "")}" data-index-Id="${result.length === 0 ? i : result.length}">
                                           </div>`);
                }
