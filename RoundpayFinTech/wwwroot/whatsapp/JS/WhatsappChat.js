...
//#endregion
function ChatArea(ad, mn, aitid, apcde, sndno,sndnoid) {
    $("#loaderimage").show();
    $('#hdnactcntct').val("cid" + aitid);
    $("#hdnapcde").val(apcde);
    _load(mn, ad, sndnoid).then(function () {
...
var _ContactSearch = (searchvalue, isFirstCall = true) => new Promise((resolve, reject) => {
    var hdnsno = $('#hdnsnno').val() === '' ? 0 : $('#hdnsnno').val();
    $.post('/ContactSearch', { SearchValue: searchvalue, Usmsgs: $('#chkUnseenMsg').is(':checked') === true ? 1 : 0, sid: hdnsno, isFirstCall: isFirstCall })
        .done(result => {
            if (typeof result === 'object') {
                let parentDiv = $('#searchcontact').find('.ContacstScroll');
                parentDiv.empty();
                for (let i = 0; i < result.getWhatsappContactList.length; i++) {
                    ...
                    parentDiv.append(`<div data-chat-no="${dataChatNo}" class="wrapBox">
                                                <a id="contactid2" onclick="ChatArea(${obj.apiid},'${obj.mobileNo}',${count},'${obj.apICode}','${obj.senderMobileNo}','${obj.senderNoID}')">
            ...
            if (txtsa === '' && $('#chkUnseenMsg').is(':checked') === false) {
                ...
                if ($('#hdnactcntct').val() !== '') {
                    $('.chat-list-item').css({ "background": "#FFFFFF" });
                    $('#' + hdnactcntct).css({ "background": "#ebebeb" });
                }
            }
...
$(document).ready(function () {
    ...
    function refresh() {
        var div = $('#messages'),
            divHtml = div.html();
        _load($("#lblSenderMobile").text(), $("#lblapiid").text(), $("#lblSenderNoID").text()).then(function () {
            _ContactSearch($('#txtsearch').val(), false);
        });
        div.html();
    }
        ...
        $.ajax({
            ...
            success: result => {
                ...
                _ContactSearch($('#txtsearch').val(), false).then(function () {
                    _load(no, aid, SenderNoID).then(function () {
                        _load(no, aid, SenderNoID);
                    });
                });
                $("#loaderimage").hide();
            }
...
var _load = (mobileNo, AIID,SndNoID) => new Promise((resolve, reject) => {
    $.post('/WpChatArea', { Mobile: mobileNo, AID: AIID, sndnoid: SndNoID })
        .done(function (result) {
...
function WhatsappTask() {
    ...
    $.post('/WpChatArea', { contactid: hid, IsTask: AIID, sndnoid: SndNoID })
        .done(function (result) {
            $('#messages').html(result);
            $("#loaderimage").hide();
            $('#bottomscroll').scrollTop($('#bottomscroll')[0].scrollHeight);
